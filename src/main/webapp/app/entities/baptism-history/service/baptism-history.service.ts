...
type RestOf<T extends IBaptismHistory | NewBaptismHistory> = Omit<T, 'baptismDate' | 'baptisedAt' | 'confirmationDate' | 'marriageDate'> & {
...
export class BaptismHistoryService {
  ...
  protected convertDateFromClient<T extends IBaptismHistory | NewBaptismHistory | PartialUpdateBaptismHistory>(
    ...
    return {
      ...
      baptisedAt: baptismHistory.baptisedAt?.format(DATE_FORMAT) ?? null,
      confirmationDate: baptismHistory.confirmationDate?.format(DATE_FORMAT) ?? null,
      marriageDate: baptismHistory.marriageDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restBaptismHistory: RestBaptismHistory): IBaptismHistory {
    return {
      ...
      baptisedAt: restBaptismHistory.baptisedAt ? dayjs(restBaptismHistory.baptisedAt) : undefined,
      confirmationDate: restBaptismHistory.confirmationDate ? dayjs(restBaptismHistory.confirmationDate) : undefined,
      marriageDate: restBaptismHistory.marriageDate ? dayjs(restBaptismHistory.marriageDate) : undefined,
    };
  }
